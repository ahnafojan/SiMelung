 ...
 */
class BumdesRekapPetani extends BaseController
{
    use ResponseTrait;
    ...
    private function _buildPetaniListView($petaniData, $pager)
    {
        ...
        foreach ($petaniData as $petani) {
            $mobileHtml .= '<div class="request-card">' .
                ...
                '<p class="mb-0"><strong><i class="fas fa-seedling fa-fw mr-2"></i>Jenis Kopi:</strong> ' . esc($petani['jenis_kopi_list'] ?? 'Tidak Terdata') . '</p>' .
                '</div></div>';
        }
        $mobileHtml .= '</div></div>';
