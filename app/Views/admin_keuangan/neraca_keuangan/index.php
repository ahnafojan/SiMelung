...
<div class="container-fluid">
    ...
    <?php if (isset($tahunDipilih)): ?>
        ...
        <form action="<?= site_url('neraca-keuangan/simpan'); ?>" method="post" id="neraca-form">
            ...
            <div class="row d-none d-lg-flex" id="desktop-view">
                <div class="col-lg-6">
                    <div class="card shadow mb-4">
            ...
            <div class="d-lg-none" id="mobile-view">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function calculateNeraca() {
                // [PERBAIKAN] Tentukan container aktif dan tidak aktif
                const desktopView = document.getElementById('desktop-view');
                const mobileView = document.getElementById('mobile-view');

                const isActiveDesktop = window.getComputedStyle(desktopView).display !== 'none';

                const activeContainer = isActiveDesktop ? desktopView : mobileView;
                const inactiveContainer = isActiveDesktop ? mobileView : desktopView;

                // [PERBAIKAN] Nonaktifkan input di container yang tidak terlihat agar tidak terkirim
                if (inactiveContainer) {
                    inactiveContainer.querySelectorAll('input.input-rupiah').forEach(input => input.disabled = true);
                }
                // [PERBAIKAN] Aktifkan input di container yang terlihat
                if (activeContainer) {
                    activeContainer.querySelectorAll('input.input-rupiah').forEach(input => input.disabled = false);
                }

                if (!activeContainer) return; // Keluar jika tidak ada container

                // --- KALKULASI AKTIVA (hanya dari container aktif) ---
                let totalAktivaLancar = 0;
                activeContainer.querySelectorAll('[data-kategori="aktiva_lancar"]').forEach(input => totalAktivaLancar += unformatRupiah(input.value));
                document.querySelectorAll('.total-aktiva-lancar').forEach(el => el.value = formatRupiah(totalAktivaLancar));

                let totalAktivaTetap = 0;
                activeContainer.querySelectorAll('[data-kategori="aktiva_tetap"]').forEach(input => totalAktivaTetap += unformatRupiah(input.value));
                ...
                // --- KALKULASI PASIVA (hanya dari container aktif) ---
                let totalHutangLancar = 0;
                activeContainer.querySelectorAll('[data-kategori="hutang_lancar"]').forEach(input => totalHutangLancar += unformatRupiah(input.value));
                ...
                activeContainer.querySelectorAll('[data-kategori="hutang_jangka_panjang"]').forEach(input => totalHutangJangkaPanjang += unformatRupiah(input.value));
                ...
                const surplusDefisitInput = activeContainer.querySelector('.surplus-defisit');
                if (surplusDefisitInput) {
                    surplusDefisit = unformatRupiah(surplusDefisitInput.value);
                }

                let totalModalDinamis = 0;
                activeContainer.querySelectorAll('[data-kategori="modal"]').forEach(input => totalModalDinamis += unformatRupiah(input.value));
