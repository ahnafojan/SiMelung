...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Formulir Edit Jenis Pendapatan</h6>
        </div>
        <div class="card-body">
            <form action="<?= site_url('/master-pendapatan/' . $pendapatan['id']); ?>" method="post">
                <?= csrf_field(); ?>
                <input type="hidden" name="_method" value="PUT">
                <div class="mb-3">
                    <label for="nama_pendapatan" class="form-label">Nama Pendapatan</label>
                    <input type="text" class="form-control <?= ($validation->hasError('nama_pendapatan')) ? 'is-invalid' : ''; ?>" id="nama_pendapatan" name="nama_pendapatan" value="<?= old('nama_pendapatan', $pendapatan['nama_pendapatan']); ?>" autofocus>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="deskripsi" name="deskripsi" rows="3"><?= old('deskripsi', $pendapatan['deskripsi']); ?></textarea>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const id_pendapatan = <?= $pendapatan['id']; ?>;
        ...
        async function checkNamaPendapatan() {
            ...
            const response = await fetch(`<?= site_url('/master-pendapatan/check-nama') ?>?nama_pendapatan=${encodeURIComponent(nama)}&id=${id_pendapatan}`);
